...
const movies = await fetch("http://www.omdbapi.com/?s=harry&apikey=35989762");
console.log(movies);
const parsedMovies = await movies.json();
console.log(parsedMovies);
const moviesArray = parsedMovies.Search;
...
//{movies[i].times.map((time,index)=>{
//  return <TouchableHighlight key={index}><Text>{time}</Text></TouchableHighlight>
//})}
//{movies[i].days.map((time,index)=>{
//  return <TouchableHighlight key={index}><Text>{time}</Text></TouchableHighlight>
//})}
